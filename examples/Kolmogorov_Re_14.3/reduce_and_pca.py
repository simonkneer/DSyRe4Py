...
try:
    ...
    if user_input.lower() == "y":
        ...
        import shutil
        URL = "https://zenodo.org/records/15195938/files/kolmogorov_Re_14.3_64x64_dt_0.1.h5?download=1"
...
data = np.array(hf.get('vorticity')[:,:,:])
hf.close()

kom = kolmo_stuff(data,Re=Re)
#define trajectory length for dynamic inversion 
length_traj  = 250#int(250/dt)
max_modes = 10
modes_step = 1

T = np.linspace(0,length_traj*dt,length_traj)

sr = symmred([translate],[1],[shift_reflect,rotate],[4,2],data.shape,weight_path=weight_path,RHS=kom.get_deriv_ts,spatial_derivs=[kom.get_deriv_x_ts],dt=dt)
...
fig_eigs.savefig('fig/kolmo_period_eigs.pdf')
...
fig_proj.savefig('fig/kolmo_period_proj.pdf')
...
fig_ts_err.savefig('fig/kolmo_period_ts_error.pdf')
...
figq.tight_layout()
figq.savefig('fig/kolmo_period_error.pdf')
plt.show()
